...
public static partial class Day1
{
    ...
    private static partial Regex DigitOrStringNumberRegex();

    [GeneratedRegex(_digitOrStringNumberPattern, RegexOptions.RightToLeft)]
    private static partial Regex DigitOrStringNumberReverseRegex();
    ...
    public static int Part2()
    {
        ...
        {
            var matches = DigitOrStringNumberRegex().EnumerateMatches(lineSpan);
            matches.MoveNext();
            var first = matches.Current;

            matches = DigitOrStringNumberReverseRegex().EnumerateMatches(lineSpan);
            matches.MoveNext();
            var last = matches.Current;

            sum += EvaluateMatch(lineSpan[first.Index..(first.Index+first.Length)]) * 10
                   + EvaluateMatch(lineSpan[last.Index..(last.Index+last.Length)]);
        }

        return sum;

        static int EvaluateMatch(ReadOnlySpan<char> m) =>
            m switch
            {
                {Length: 1} => m[0] - '0',
                "one" => 1,
