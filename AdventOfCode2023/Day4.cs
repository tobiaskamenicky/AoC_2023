...
public static class Day4
{
    ...
    public static int Part1()
    {
        ...
        foreach (var line in _input.AsSpan().EnumerateLines())
        {
            ...
            var winningNumbers = line[start..delimiter];
            var myNumbers = line[(delimiter + 1)..];

            var points = 1;
            for (var s = 0; s < myNumbers.Length - 1; s += 3)
            {
                var number = myNumbers[s..(s + 3)];
                var isWinning = winningNumbers.IndexOf(number) != -1;
                if (isWinning) points <<= 1;
            }
    ...
    public static int Part2()
    {
        ...
        // Count on input being quite small
        Span<int> cardCopies = stackalloc int[_input.AsSpan().Count(':')];
        cardCopies.Fill(1);
        ...
        foreach (var line in _input.AsSpan().EnumerateLines())
        {
            ...
            for (var s = 0; s < myNumbers.Length - 1; s += 3)
            {
                ...
                {
                    ++wins;
                    cardCopies[lineIndex + wins] += cardCopies[lineIndex];
                }
            }

            result += cardCopies[lineIndex];
        }
