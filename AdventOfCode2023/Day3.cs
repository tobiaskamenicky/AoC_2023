...
public static class Day3
{
    ...
    private static readonly SearchValues<char> _searchDigitOrDot = SearchValues.Create("0123456789.");
    ...
    public static int Part1()
    {
        ...
        var lineLength = input.IndexOf(Environment.NewLine[0]) + Environment.NewLine.Length;
        ...
        foreach (var line in input.EnumerateLines())
        {
            ...
            var numberStart = line.IndexOfAnyInRange('0', '9');
            while(true)
            {
                if (numberStart == -1) break;
                var numberLength = line[numberStart..].IndexOfAnyExceptInRange('0', '9');
                var numberEnd = numberLength == -1 ? line.Length : numberStart + numberLength;

                var checkStart = Math.Max(0, numberStart - 1);
                var checkEnd = Math.Min(numberEnd + 1, line.Length - 1);
                var include = false;
                if (lineIndex != 0)
                {
                    var prevLine = input[((lineIndex - 1) * lineLength)..((lineIndex - 1) * lineLength + line.Length)];
                    var prevLineSlice = prevLine[checkStart..checkEnd];
                    if (prevLineSlice.IndexOfAnyExcept(_searchDigitOrDot) != -1)
                    {
                        include = true;
                    }
                }

                if (!_searchDigitOrDot.Contains(line[checkStart]) || !_searchDigitOrDot.Contains(line[checkEnd - 1]))
                {
                    include = true;
                }

                var nextLineStartIndex = (lineIndex + 1) * lineLength;
                if (nextLineStartIndex < input.Length)
                {
                    var nextLine = input[nextLineStartIndex..(nextLineStartIndex + line.Length)];
                    var nextLineSlice = nextLine[checkStart..checkEnd];
                    if (nextLineSlice.IndexOfAnyExcept(_searchDigitOrDot) != -1)
                    {
                        include = true;
                    }
                }
                ...
                if (include)
                {
                    result += int.Parse(line[numberStart..numberEnd]);
                }

                var nextNumberStart = line[numberEnd..].IndexOfAnyInRange('0', '9');
                if (nextNumberStart == -1) break;
                numberStart = numberEnd + nextNumberStart;
            }
        }
    ...
    public static int Part2()
    {
        ...
        Span<int> nNumbers = stackalloc int[6];
        ...
        foreach (var line in input.EnumerateLines())
        {
            ...
            while (true)
            {
                ...
                if (symbolIndex == line.Length - 1) break;
                var nextSymbolIndex = line[(symbolIndex + 1)..].IndexOf('*');
                if (nextSymbolIndex == -1) break;
                symbolIndex += nextSymbolIndex + 1;

                // Not checking boundaries because I know the input
                //check previous
                var previousLineStart = (lineIndex - 1) * lineLength;
                var previousLine = input[previousLineStart..(previousLineStart + lineLength)];
                (nNumbers[0], nNumbers[1]) = FindNumbers(previousLine, symbolIndex);

                // check current
                (nNumbers[2], nNumbers[3]) = FindNumbers(line, symbolIndex);

                // check next
                var nextLineStart = (lineIndex + 1) * lineLength;
                var nextLine = input[nextLineStart..(nextLineStart + line.Length)];
                (nNumbers[4], nNumbers[5]) = FindNumbers(nextLine, symbolIndex);

                if (nNumbers.Count(1) == 4)
                {
                    var power = 1;
            ...
            static int FindNumberEnd(ReadOnlySpan<char> span)
            {
                var index = span.IndexOfAnyExceptInRange('0', '9');
                return index == -1 ? span.Length : index;
            }

            static int FindNumberStart(ReadOnlySpan<char> span) => span.LastIndexOfAnyExceptInRange('0', '9') + 1;

            static (int, int) FindNumbers(ReadOnlySpan<char> line, int checkIndex)
            {
                var startI = Math.Max(checkIndex - 1, 0);
                var endI = Math.Min(checkIndex + 1, line.Length);
                var leftPart = line[..(startI + 1)];
                var rightPart = line[endI..];
                return line[startI..(endI + 1)] switch
                {
                    [>= '0' and <= '9', >= '0' and <= '9', >= '0' and <= '9'] => (1, int.Parse(line[FindNumberStart(leftPart)..(endI + FindNumberEnd(rightPart))])),
                    [>= '0' and <= '9', >= '0' and <= '9', _] => (int.Parse(line[FindNumberStart(leftPart)..(startI + 2)]), 1),
                    [>= '0' and <= '9', _, >= '0' and <= '9'] => (int.Parse(line[FindNumberStart(leftPart)..(startI + 1)]), int.Parse(line[(endI)..(endI + FindNumberEnd(rightPart))])),
                    [>= '0' and <= '9', _, _] => (int.Parse(line[FindNumberStart(leftPart)..(startI + 1)]), 1),
                    [_, >= '0' and <= '9', >= '0' and <= '9'] => (1, int.Parse(line[(endI - 1)..(endI + FindNumberEnd(rightPart))])),
                    [_, >= '0' and <= '9', _] => (line[startI + 1] - '0', 1),
                    [_, _, >= '0' and <= '9'] => (1, int.Parse(line[(endI)..(endI + FindNumberEnd(rightPart))])),
                    _ => (1, 1),
                };
            }
        }
