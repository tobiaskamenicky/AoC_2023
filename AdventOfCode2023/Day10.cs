...
public static class Day10
{
    public static void Solve()
    {
        ...
        Console.WriteLine($"Part 1: {Part1()}"); //7066
        Console.WriteLine($"Part 2: {Part2()}"); //401
    }

    public static int Part1()
    {
        var input = _input.AsSpan();
        var width = input.IndexOf(Environment.NewLine[0]) + Environment.NewLine.Length;

        // Get how to start
        var start = input.IndexOf('S');
        Span<(int, char, char, char)> directions = stackalloc[]
        {
            (start - width, '|', '7', 'F'),
            (start + width, '|', 'L', 'J'),
            (start - 1, '-', 'L', 'F'),
        ...
        {
            var (index, c1, c2, c3) = directions[i];
            var v = input[index];
            if (index < 0 || index >= input.Length || (c1 != v && c2 != v && c3 != v))
            {
                continue;
            }
        ...
        do
        {
            direction = GetNextDirection(input[next], direction);
        ...
        static int GetNextDirection(char current, int cameFrom)
        {
            return (current, cameFrom) switch
            {
                ('|', _) => cameFrom,
                ('-', _) => cameFrom,
                ('L', 1) => 3,
    ...
    public static long Part2()
    {
        ...
        var inLoop = new HashSet<int> {start};
        var last = start;
        var sum = 0d;
        do
        {
            // Determine if the loop is clockwise or counter-clockwise
            var p1 = GetCoordinates(last, width);
            var p2 = GetCoordinates(next, width);
            sum += p1.X * p2.Y - p2.X * p1.Y;

            // Remember all positions in loop
            inLoop.Add(next);
            last = next;
        ...
        var pFirst = GetCoordinates(last, width);
        var pLast = GetCoordinates(next, width);
        sum += pLast.X * pFirst.Y - pFirst.X * pLast.Y;
        var clockwise = sum >= 0;

        var enclosed = 0;
        direction = startDirection;
        next = startNext;
        // Everything enclosed is on the right if clockwise, on the left if counter-clockwise
        do
        {
            var checkRight = (input[next], direction, clockwise) switch
            {
                ('|', 0, true) => true,
                ('|', 1, false) => true,
                ('J', 3, true) => true,
            ...
            {
                var nextP = next + 1;
                while (input[nextP] != Environment.NewLine[0])
                {
                    if (inLoop.Contains(nextP))
                    {
                        enclosed += nextP - next - 1;
                        break;
                    }
        ...
        static (int X, int Y) GetCoordinates(int index, int width)
        {
    ...
    private const string _testInput2 = """
                                       .F----7F7F7F7F-7....
                                       .|F--7||||||||FJ....
                                       .||.FJ||||||||L7....
                                       FJL7L7LJLJ||LJ.L-7..
    ...
    private const string _testInput3 = """
                                       FF7FSF7F7F7F7F7F---7
                                       L|LJ||||||||||||F--J
                                       FL-7LJLJ||||||LJL-77
                                       F--JF--7||LJLJ7F7FJ-
