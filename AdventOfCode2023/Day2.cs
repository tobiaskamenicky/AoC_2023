...
{
    private static readonly SearchValues<char> _searchDelimiter = SearchValues.Create(";,:");

    private static int GetColorIndex(char c) => c switch
    {
        'r' => 0,
        'g' => 1,
        'b' => 2,
    ...
    public static int Part1()
    {
        ...
        Span<int> gameMax = stackalloc int[3];
        ...
        foreach (var line in _input.AsSpan().EnumerateLines())
        {
            ...
            while (true)
            {
                var nextIndex = rest.IndexOfAny(_searchDelimiter) + 2;
                if (nextIndex == 1)
                {
                    break;
                }
                ...
                var number = int.Parse(rest[..spaceIndex]);
                var colorIndex = GetColorIndex(rest[spaceIndex + 1]);
                if (number > gameMax[colorIndex])
                {
                    gameMax[colorIndex] = number;
                }
            }

            if (gameMax is [< 13, < 14, < 15])
            {
    ...
    public static int Part2()
    {
        ...
        foreach (var line in _input.AsSpan().EnumerateLines())
        {
            ...
            power += gameMax[0] * gameMax[1] * gameMax[2];
        }
