...
public static class Day5
{
    ...
    public static long Part1()
    {
        ...
        var count = lineEnumerator.Current.Count(' ');
        Span<Range> ranges = stackalloc Range[count + 1];
        Span<long> values = stackalloc long[count];
        Span<bool> mapped = stackalloc bool[count];

        lineEnumerator.Current.Split(ranges, ' ');
        for (var i = 0; i < count; i++)
        {
            values[i] = long.Parse(lineEnumerator.Current[ranges[i + 1]]);
        }

        Span<long> mapValues = stackalloc long[3];
        ...
        while (lineEnumerator.MoveNext())
        {
            mapped.Clear();
            lineEnumerator.MoveNext();
            while (lineEnumerator.Current.Length > 0)
            {
                var line = lineEnumerator.Current;
                line.Split(mapRanges, ' ');
                for (var i = 0; i < 3; i++)
                {
                    mapValues[i] = long.Parse(line[mapRanges[i]]);
                }

                var sourceS = mapValues[1];
                var sourceE = mapValues[1] + mapValues[2];
                for (var i = 0; i < count; i++)
                {
                    if (!mapped[i] && values[i] >= sourceS && values[i] < sourceE)
                    {
                        var diff = values[i] - sourceS;
                        values[i] = mapValues[0] + diff;
                        mapped[i] = true;
                    }
                }
    ...
    public static long Part2()
    {
        ...
        var inputCount = lineEnumerator.Current.Count(' ');
        var maxValuesCount = inputCount * 10;
        var valuesCount = 0;
        Span<Range> ranges = stackalloc Range[inputCount + 1];
        Span<(long Start, long Length)> values = stackalloc (long, long)[maxValuesCount];
        Span<bool> mapped = stackalloc bool[maxValuesCount];

        lineEnumerator.Current.Split(ranges, ' ');
        for (var i = 0; i < inputCount; i += 2)
        {
            var start = long.Parse(lineEnumerator.Current[ranges[i + 1]]);
            var length = long.Parse(lineEnumerator.Current[ranges[i + 2]]);
            values[valuesCount] = (start, length);
            mapped[valuesCount] = false;
            ++valuesCount;
        }
        ...
        while (lineEnumerator.MoveNext())
        {
            ...
            while (lineEnumerator.Current.Length > 0)
            {
                ...
                for (var i = 0; i < valuesCount; i++)
                {
                    ...
                    var start = values[i].Start;
                    var end = values[i].Start + values[i].Length;

                    if (end < sourceS) continue;
                    if (sourceE < start) continue;

                    if (start < sourceS && sourceS < end)
                    {
                        var newRange = (start, sourceS - start);
                        values[valuesCount] = newRange;
                        ++valuesCount;

                        values[i] = (sourceS, values[i].Length - newRange.Item2);
                        start = values[i].Start;
                        end = values[i].Start + values[i].Length;
                    }

                    if (start < sourceE && sourceE < end)
                    {
                        var newRange = (sourceE, end - sourceE);
                        values[valuesCount] = newRange;
                        ++valuesCount;

                        values[i] = (values[i].Start, values[i].Length - newRange.Item2);
                        start = values[i].Start;
                        end = values[i].Start + values[i].Length;
                    }

                    if (sourceS <= start && end <= sourceE)
                    {
                        var diff = start - sourceS;
                        values[i] = (mapValues[0] + diff, values[i].Length);
                        mapped[i] = true;
                    }
                }
        ...
        for (var i = 0; i < valuesCount; i++)
        {
            result = Math.Min(values[i].Start, result);
        }
